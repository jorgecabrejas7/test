...
export default () => {
    ...
    useEffect(() => {
        if(id > 0 && session.status === "authenticated") {
            const fetchData = async () => {
                let res = await protectedFetch(session, process.env.BACK_API_URL + "ticket?" + new URLSearchParams({ id_ticket: id }), "get", null, t);
                if(res.status === 200) {
                    const data = await res.json();
                    setTicket(data);
                }

                res = await protectedFetch(session, process.env.BACK_API_URL + "ticket/messages?" + new URLSearchParams({ id_ticket: id }), "get", null, t);
                if(res.status === 200) {
                    const data = await res.json();
                    setMessages(data);
                }
    ...
    const handleSubmit = async e => {
        ...
        const res = await protectedFetch(
            session, process.env.BACK_API_URL + "ticket/message?" + new URLSearchParams({ id_ticket: id }),
        ...
        if(res.status === 200) {
            ...
            cloneTicket.status = 'waiting_response';
    ...
    const toggleTicket = async (toggle) => {
        ...
        const status = toggle ? 'waiting_response' : 'finished';
        const res = await protectedFetch(
            session, process.env.BACK_API_URL + "ticket?" + new URLSearchParams({ id_ticket: id }),
